	...
	Graph* Graph::loadGraph(const char* fileName, int my_rank) {
	...
	const char* Graph::serialize() const {
		return matrix->serialize();
	}
	
	void Graph::deserialize(const char* _data) {
		matrix->deserialize(_data);
	}
